...
// Known embed origins that send postMessage events
...
export const VideoEmbedPlayer = ({ media, initialSeason = 1, initialEpisode = 1 }: VideoEmbedPlayerProps) => {
    ...
    useEffect(() => {
        function handleKey(e: KeyboardEvent) {
            ...
            switch (e.key) {
                case ' ':
                    e.preventDefault();
                    // Toggle play state visually; actual control is inside iframe
                    setIsPlaying(p => !p);
                    break;
                case 'ArrowRight':
                    e.preventDefault();
                    if (media.media_type === 'tv') {
                        ...
                        if (activeEpisode < maxEp) setActiveEpisode(e => e + 1);
                    }
                    break;
                case 'ArrowLeft':
                    e.preventDefault();
                    if (media.media_type === 'tv' && activeEpisode > 1) setActiveEpisode(e => e - 1);
                    break;
                case 'ArrowUp':
                    e.preventDefault();
                    if (media.media_type === 'tv') {
                        const maxS = media.seasons?.length ?? 1;
                        if (activeSeason < maxS) { setActiveSeason(s => s + 1); setActiveEpisode(1); }
                    }
                    break;
                case 'ArrowDown':
                    e.preventDefault();
                    if (media.media_type === 'tv' && activeSeason > 1) { setActiveSeason(s => s - 1); setActiveEpisode(1); }
                    break;
            }
        }
    ...
    // postMessage from iframe — origin-validated
    useEffect(() => {
    ...
    // Auto-start tracking after 3s fallback
    useEffect(() => {
        ...
        fetch('/api/continue-watching')
            ...
            .then((items: Array<{ tmdb_id: string; season_number?: number; episode_number?: number; progress_seconds?: number; duration_seconds?: number }>) => {
                ...
                for (const item of items) {
                    if (item.tmdb_id === String(media.id) && item.season_number && item.episode_number) {
                        const pct = item.duration_seconds ? (item.progress_seconds ?? 0) / item.duration_seconds : 0;
                        map[`${item.season_number}-${item.episode_number}`] = pct;
                    }
                }
        ...
        try {
            await fetch('/api/continue-watching', {
                ...
                body: JSON.stringify({
                    ...
                    // Use TMDB runtime if available
                    duration_seconds: media.media_type === 'movie'
                        ? (media.runtime ?? 0) * 60
        ...
        <div className="w-full">
            ...
            <div className="aspect-video bg-black relative">
                <AnimatePresence mode="wait">
                    <motion.iframe
                        ...
                        // allow-fullscreen added, allow-same-origin REMOVED (security fix)
                        // allow-popups needed for some providers to init their player
            ...
            <div className="bg-gray-900 px-4 py-3 flex flex-wrap items-center justify-between gap-2 border-b border-gray-800">
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-gray-500 text-xs mr-1 hidden sm:inline">Server:</span>
                    {servers.map(server => (
                        <button
                            key={server.key}
                            onClick={() => setActiveServer(server.key)}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${activeServer === server.key ? 'bg-red-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'}`}>
                            {server.name}
                        </button>
                    ))}
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {hasNextEpisode && (
                        <button
                            onClick={() => setActiveEpisode(ep => ep + 1)}
                            className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-md bg-white text-black hover:bg-gray-200 transition-colors">
                            Next Ep →
                        </button>
                    )}
                    {officialTrailer && (
            ...
            <div className="bg-gray-950 px-4 py-2 hidden md:flex items-center gap-4 text-gray-600 text-xs border-b border-gray-800">
                {media.media_type === 'tv' && (
                    <>
                        <span><kbd className="bg-gray-800 text-gray-400 px-1 rounded">←</kbd><kbd className="bg-gray-800 text-gray-400 px-1 rounded ml-0.5">→</kbd> Episode</span>
                        <span><kbd className="bg-gray-800 text-gray-400 px-1 rounded">↑</kbd><kbd className="bg-gray-800 text-gray-400 px-1 rounded ml-0.5">↓</kbd> Season</span>
                    </>
                )}
            ...
            {(media.genres ?? []).length > 0 && (
                <div className="px-4 py-3 flex flex-wrap gap-2 border-b border-gray-800">
                    {media.genres!.map((genre) => (
                        <span key={genre.id} className="px-2 py-1 text-xs rounded-full bg-gray-800 text-gray-400">
                            {genre.name}
                        </span>
                    ))}
                </div>
            ...
            {media.media_type === 'tv' && media.seasons && (
                <div className="p-4 md:px-8 md:py-6">
                    {/* Season tabs */}
                    <div className="flex gap-1 border-b border-gray-800 mb-4 overflow-x-auto pb-0 scrollbar-hide">
                        ...
                        {Array.from({ length: currentSeason?.episode_count ?? 0 }, (_, i) => i + 1).map(ep => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative min-h-[48px] rounded-md text-sm font-medium transition-all overflow-hidden ${isActive
                                    ...
                                    {isPartial && !isActive && (
                                        <span className="absolute bottom-0 left-0 h-0.5 bg-red-500 transition-all" style={{ width: `${pct * 100}%` }} />
                                    )}
                                    {/* Watched tick */}
                                    {isWatched && !isActive && (
                    ...
                    <p className="text-gray-600 text-xs mt-3">
                        Season {activeSeason} · Episode {activeEpisode} of {currentSeason?.episode_count ?? '?'}
